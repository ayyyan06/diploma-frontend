...
import {
  ...
  buildSteppeRoadResult,
  getSteppeRoadOptionLabel,
  saveSteppeRoadResult,
} from "../../data/steppeRoadTest";

export const SteppeRoadQuestion = () => {
  ...
  const currentScene = STEPPE_ROAD_SCENES[currentSceneIndex];
  const currentSceneNumber = currentSceneIndex + 1;
  const selectedAnswer = selectedAnswers[currentScene.id];
  const isLastScene = currentSceneNumber === STEPPE_ROAD_SCENES.length;
  const isFirstScene = currentSceneIndex === 0;
  const progressValue = useMemo(
    () => (currentSceneNumber / STEPPE_ROAD_SCENES.length) * 100,
    [currentSceneNumber],
  );

  const previousScene =
    currentSceneIndex > 0 ? STEPPE_ROAD_SCENES[currentSceneIndex - 1] : null;
  const previousChoiceLabel = previousScene
    ? getSteppeRoadOptionLabel(
        previousScene.id,
        selectedAnswers[previousScene.id] ?? "",
      )
    : null;
  ...
  const submitResult = async () => {
    const result = buildSteppeRoadResult(selectedAnswers);
    saveSteppeRoadResult(result);

    try {
      await fetchWithToken(
        "/api/v1/tests",
        { method: "POST" },
        { type: STEPPE_ROAD_TEST_TYPE, result },
      );
    } catch (error) {
      console.error("Failed to persist Steppe Road result, using local copy.", error);
    }

    navigate("/tests/road-result");
  };
    ...
    return (
      <main className="mx-auto mt-[72px] flex min-h-[60vh] w-full max-w-[1240px] flex-col items-center justify-center gap-4 px-[18px] pb-12">
        ...
        <p className="text-[18px] text-[#555555]">
          Reading the road and preparing your result...
        </p>
      </main>
    );
  }
    ...
    <main className="mx-auto mt-[72px] w-full max-w-[1240px] px-[18px] pb-12 max-[900px]:mt-12">
      <section className="mb-9 grid gap-[14px]" aria-label="Question progress">
        <p className="m-0 text-[20px] font-normal leading-[1.2] tracking-[0.02em] text-[#7a7a7a] max-[640px]:text-[16px]">
          SCENE {currentSceneNumber} OF {STEPPE_ROAD_SCENES.length}
        </p>
          ...
          <div className="max-w-[760px]">
            ...
            {previousChoiceLabel ? (
              <p className="mt-[16px] mb-0 max-w-[700px] rounded-[16px] bg-[#fbf7ef] px-[18px] py-[14px] text-[15px] leading-[1.55] text-[#6c655c]">
                Earlier on the road, you chose to {previousChoiceLabel.toLowerCase()}
              </p>
            ) : null}
          </div>
        ...
        <div className="mt-[42px] flex items-center justify-between gap-4 max-[640px]:flex-col">
          {!isFirstScene ? (
            <button
              ...
              className="
                min-h-[56px] w-[200px] rounded-[16px]
          ...
          <button
            ...
            className="
              ...
              text-[18px] font-bold tracking-[0.03em] text-white
